...
fn get_happiness<'a>(input: &'a String) -> HashMap<&'a str, HashMap<&'a str, i32>> {
    ...
    for l in input.lines() {
        ...
        let happiness =
            if is_gain { 1 } else { -1 } * c.get(3).unwrap().as_str().parse::<i32>().unwrap();
...
fn pt1(input: &HashMap<&str, HashMap<&str, i32>>) -> i32 {
    ...
    for perm in input.keys().permutations(input.len()) {
        ...
        while i < perm.len() {
            let a = input.get(*perm.get(i).unwrap()).unwrap();
            let i_b1 = if i == 0 { perm.len() - 1 } else { i - 1 };
            let i_b2 = if i == perm.len() - 1 { 0 } else { i + 1 };
            sub_result += a.get(*perm.get(i_b1).unwrap()).unwrap();
            sub_result += a.get(*perm.get(i_b2).unwrap()).unwrap();
            i += 1;
        }
        result = match result {
