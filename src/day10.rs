...
pub fn run() {
    ...
    let input: Vec<i32> = start.chars().map(|v| v.to_string().parse::<i32>().unwrap()).collect();
    let t1 = Instant::now();
    ...
    println!("Day10 Pt1 (String builder): {}", pt1_2_bad(input.clone(), 40));
    println!("Day10 Pt2 (String builder): {}", pt1_2_bad(input, 50));
    println!("Day10 (String builder) {:?}", Instant::now().duration_since(t2));
}
