...
fn pt2(packages: &Vec<i32>) -> u128 {
    ...
    for i in 0..packages.len() {
        for j in i + 1..packages.len() {
            for k in j + 1..packages.len() {
                if packages[i] + packages[j] + packages[k] == weight_per_compartment {
                    // weight
                    possible_3.push(vec![i, j, k]);
                }
                for l in k + 1..packages.len() {
                    if packages[i] + packages[j] + packages[k] + packages[l]
                        == weight_per_compartment
                    {
                        // weight
                    ...
                    for m in l + 1..packages.len() {
                        if packages[i] + packages[j] + packages[k] + packages[l] + packages[m]
                            == weight_per_compartment
                        {
                            // weight
